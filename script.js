...
continueButton.addEventListener('click', function() {
    ...
    function copyEmail(email) {
        navigator.clipboard.writeText(email)
            .then(() => {
                const maxLength = 25; // Giới hạn số ký tự hiển thị
                const displayEmail = email.length > maxLength ? email.substring(0, maxLength) + "..." : email;
                showCustomAlert(`Email đã được sao chép: ${displayEmail}`);
            })
            .catch(() => {
                showCustomAlert('Sao chép thất bại, vui lòng thử lại!');
            });
    ...
    (function() {
        ...
        function protectCode() {
            if (document.body.innerHTML.includes('Design by Hoang Van Bao')) {
